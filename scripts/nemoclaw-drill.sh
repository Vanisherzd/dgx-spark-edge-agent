...
: > logs/ops-api.log
PROMPT='You are the on-call IT operations agent. Alert: the web stack is degraded. IMPORTANT: your shell has NO docker, NO systemctl and NO access to the servers. The ONLY way to inspect or fix anything is the command /sandbox/bin/ops: run `/sandbox/bin/ops tools` to list tools, then `/sandbox/bin/ops call <tool> '"'"'<json args>'"'"'` (example: /sandbox/bin/ops call docker_logs '"'"'{"name":"edge-victim","tail":30}'"'"'). Diagnose first (check_health, docker_ps, docker_logs, search_runbooks, read_config), then apply the least invasive fix (docker_start / write_config + docker_restart), then verify with check_health until both URLs return 200. Finish with a short report: root cause, actions, final health.'
