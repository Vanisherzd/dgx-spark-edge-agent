...
4. Unified memory: `--gpu-memory-utilization` is a fraction of the whole 119 GiB pool and is pre-allocated for KV cache; other GPU users (docker containers, k8s pods, ollama) fight for the same pool. Host cleaned up and node cordoned on 2026-09-21.
